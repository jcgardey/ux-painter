...
class VersionView extends React.Component {
  constructor(props) {
    ...
    this.state = {
      versionName:
        this.props.version.getName() &&
        this.props.version.getName() !=
          window.refactoringManager.getOriginalVersion().getName()
          ? this.props.version.getName()
          : '',
    };
  ...
  handleSubmit() {
    if (
      ...
      window.refactoringManager.getCurrentVersion().execute();
    ...
    return (
      <div className={'container'}>
        ...
        <div className={'row uxpainter-long-row'}>
          ...
          <div className={'col-5'}>
            <Link className={'btn btn-dark'} onClick={this.handleSubmit}>
              Create
            </Link>
          </div>
        </div>
